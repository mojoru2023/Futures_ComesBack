...
def read_csv_to_chart(filename, num):
    ...
    # 提取“<CLOSE>”列（第6列，索引为5）
    closing_prices = pd.to_numeric(data[5], errors='coerce').tolist()
    ...
    ani = FuncAnimation(fig, animate, frames=len(closing_prices),
                        ...
                        repeat=False, interval=1)  # 设置较长的间隔以减慢动画速度
...
if __name__ == "__main__":
    # 替换为实际文件路径和参数
    #read_csv_to_chart("day.csv", 6)
    #read_csv_to_chart("60m.csv", 10)
    read_csv_to_chart("2025-01.csv", 100)
