...
def read_xlsx_to_chart(filename, num, volatility_window=20):
    """
    读取XLSX并绘制价格和波动率图
    ...
    for date, volatilities in daily_volatility_dict.items():
        if volatilities:  # 如果有有效的波动率数据
            ...
            print(f"日期: {date}  小于等于 {v_threshold} 的比例: {ratio_below:.2%}  大于 {v_threshold} 的比例: {ratio_above:.2%}")
...
if __name__ == "__main__":
    v_threshold = 0.0005 # 月度数据临界值更小一些就能过滤出更多的混沌
    # read_xlsx_to_chart("day.xlsx", 6)  # 日线的支撑和压力分析
    # read_xlsx_to_chart("60m.xlsx", 10)  # 60分钟的
    read_xlsx_to_chart("2025/N225minif_2025.xlsx-1.xlsx", 100)  # 3分钟线的支撑和压力分析